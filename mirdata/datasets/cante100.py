...
def load_spectrogram(faudio: TextIO) -> np.ndarray:
    ...
    """
    parsed_spectrogram = np.genfromtxt(faudio, delimiter=" ")
    spectrogram = parsed_spectrogram.astype(np.float)

    return spectrogram


def load_audio(faudio: str) -> Tuple[np.ndarray, float]:
    """Load a cante100 audio file.

    Args:
        faudio (str): path to an audio file
    ...
    """
    return librosa.load(faudio, sr=22050, mono=False)


@io.coerce_to_string_io
def load_melody(f0: TextIO) -> Optional[annotations.F0Data]:
    """Load cante100 f0 annotations

    Args:
        f0 (str or file-like): path or file-like object pointing to melody annotation file
...
@io.coerce_to_string_io
def load_notes(fnotes: TextIO) -> annotations.NoteData:
    """Load note data from the annotation files

    Args:
        fnotes (str or file-like): path or file-like object pointing to a notes annotation file
    ...
    pitches = []
    confidence = []
    reader = csv.reader(fnotes, delimiter=",")
    for line in reader:
