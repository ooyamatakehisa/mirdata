...
from mirdata import track, dataset
...
class Dataset(dataset.Dataset):
    def __init__(self, data_home=None):
        self.name = "beatles"
        self.bibtex = """@inproceedings{mauch2009beatles,
...
"""
    ...
    def load_beats(self, beats_path):
        ...
        beat_positions = self._fix_newpoint(np.array(beat_positions))
    ...
    def load_chords(self, chords_path):
    ...
    def load_sections(self, sections_path):
    ...
    def _fix_newpoint(self, beat_positions):
    ...
    class _Track(track.Track):
        ...
        def __init__(self, track_id, data_home, dataset):
            ...
            self._track_paths = self._dataset._index[track_id]
            self.beats_path = utils.none_path_join(
                [self._data_home, self._track_paths["beat"][0]]
            )
            self.chords_path = os.path.join(
        ...
        def beats(self):
            ...
            return self._dataset.load_beats(self.beats_path)

        @utils.cached_property
        def chords(self):
            """ChordData: chord annotation"""
            return self._dataset.load_chords(self.chords_path)

        @utils.cached_property
        def key(self):
            """KeyData: key annotation"""
            return self._dataset.load_key(self.keys_path)

        @utils.cached_property
        def sections(self):
            """SectionData: section annotation"""
            return self._dataset.load_sections(self.sections_path)

        @property
        def audio(self):
            """(np.ndarray, float): audio signal, sample rate"""
            return self._dataset.load_audio(self.audio_path)
